...
function aigpl_register_guten_block() {
	...
	// Block Editor Script
	wp_register_script( 'aigpl-block-js', AIGPL_URL.'assets/js/blocks.build.js', array( 'wp-blocks', 'wp-i18n', 'wp-element', 'wp-editor', 'wp-components' ), AIGPL_VERSION, true );
	wp_localize_script( 'aigpl-block-js', 'Aigplf_Block', array(
																'pro_demo_link' => 'https://demo.wponlinesupport.com/prodemo/album-and-image-gallery-plus-lightbox-pro/',
																'free_demo_link' => 'https://demo.wponlinesupport.com/album-and-image-gallery-plus-lightbox-demo/',
																'pro_link' => AIGPL_PLUGIN_LINK,
															));
...
function aigpl_add_block_category( $categories ) {
	...
	if( ! in_array( 'wpos_guten_block', $guten_cats ) ) {
		$categories[] = array(
							...
							'title'	=> __('WPOS Blocks', 'album-and-image-gallery-plus-lightbox'),
							'icon'	=> null,
						);
	}
...
add_filter( 'block_categories', 'aigpl_add_block_category' );